package com.example.bikram.database;
...
public class MainActivity extends Activity implements  OnClickListener{
    ...
    public void onClick(View v)
    {
        ...
        if(v==btnDelete)
        {
            ...
            {
                showMessage("error","enter rol no");
                return;
            }
        ...
        if(v== btnModify)
        {
            if(editRollno.getText().toString().trim().length()==0)
            {
                showMessage("Error","Enter the roll no");
                return;
            }
            Cursor c= db.rawQuery("Select * from student where rollno='"+editRollno.getText()+"'", null);
            if(c.moveToFirst())
            {
                ...
                showMessage("done","record updated");
            }
            else
            {
                showMessage("Error", "Invalid Rollno");
        ...
        if(v== btnView)
        {
            if(editRollno.getText().toString().trim().length()==0)
            {
                showMessage("Error","fill roll no");
                return;
            }
            Cursor c=db.rawQuery("SELECT * FROM student WHERE rollno='"+editRollno.getText()+"'", null);
            if(c.moveToFirst())
        ...
        if(v==btnShowInfo)
        {
            showMessage("DATABASE OF STUDENT","Created by Bikramjit singh");
        }
        if(v==btnViewAll)
        {
